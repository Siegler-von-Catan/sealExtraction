...
def getMostLikelyMaskFor(allContours, contoursPairs, referenceImage, numberOfWaxPixels, original):
    pairMasks = [getMasksForContourPair(allContours[pair[0]], allContours[pair[1]], referenceImage, numberOfWaxPixels)
                 for pair in contoursPairs]
    ...
    if all(len(element) == 0 for element in pairMasks):
        ...
        pairMasks = [
            getMasksForContourPair(allContours[pair[0]], allContours[pair[1]], referenceImage, numberOfWaxPixels,
                                   enable_filter=False) for pair in contoursPairs]
        pairMasks = list(filter(lambda masks: len(masks) > 0, pairMasks))
        if not pairMasks:
            print("This image can not be cropped properly, using whole image as mask")
            boxMask = np.full(referenceImage.shape[:2], 255, np.uint8)
            return boxMask
